...
type WeixinConfig struct {
	...
	Contact           WorkwxContactConfig `mapstructure:"contact"` // 通讯录同步配置
	Event             WeixinEventConfig   `mapstructure:"event"`
	Micro             WeixinMicroConfig   `mapstructure:"micro"`
	Bots              OpenclawConfig      `mapstructure:"bots"`
	Goclaw            GoclawConfig        `mapstructure:"goclaw"`
...
func (cfg *WeixinConfig) Validate() error {
	...
	if err := cfg.Bots.Validate(); err != nil {
		return err
	}
	if err := cfg.Goclaw.Validate(); err != nil {
		return err
	}
	return nil
}
...
// WorkwxContactConfig 通讯录同步助手配置
type WorkwxContactConfig struct {
	Enabled           bool   `mapstructure:"enabled"`             // 是否启用通讯录同步
	ContactSyncSecret string `mapstructure:"contact_sync_secret"` // 通讯录同步 Secret
	TxSubscribe       string `mapstructure:"tx_subject"`          // 发送消息主题
	RxSubscribe       string `mapstructure:"rx_subject"`          // 接收消息主题
	PublishChanges    bool   `mapstructure:"publish_changes"`     // 是否发布通讯录变更到 NATS
	ChangeSubject     string `mapstructure:"change_subject"`      // 通讯录变更事件主题
}
